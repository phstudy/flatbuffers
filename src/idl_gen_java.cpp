 ...
 public:
  ...
  void GenPackUnPack_ObjectAPI(
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      auto start = "    _o." + field.name + " = ";
      switch (field.value.type.base_type) {
        case BASE_TYPE_STRUCT: {
          auto fixed = struct_def.fixed && field.value.type.struct_def->fixed;
          if (fixed) {
            code += start + "this." + field.name + ".unPack();\n";
          } else {
            code += start + "this." + field.name + "() != null ? this." +
                field.name + "().unPack() : null;\n";
          }
          break;
        }
        case BASE_TYPE_ARRAY: {
        ...
        case BASE_TYPE_VECTOR:
          if (field.value.type.element == BASE_TYPE_UNION) {
            ...
            code += "    for (int _j = 0; _j < this." + field.name +
                    "Length; ++_j) {\n";
            ...
            code += "    for (int _j = 0; _j < this." + field.name +
                    "Length(); ++_j) {";
            code += "_o." + field.name + ".add(";
            if (fixed) {
              code += "this." + field.name + "(_j)";
            } else {
              code += "this." + field.name + "(_j) != null ? this." +
                  field.name + "(_j).unPack() : null";
            }
            code += ");}\n";
          }
          break;
        ...
        case BASE_TYPE_VECTOR: {
          if (field_has_create.find(&field) != field_has_create.end()) {
            ...
            code += "      start" + camel_name + "Vector(builder, _o." +
                field.name + ".size());\n";
            code += "      for (int _j = _o." + field.name +
                    ".size() - 1; _j >= 0; --_j) { " + pack_method + " }\n";
  ...
  void GenStruct_ObjectAPI(StructDef &struct_def, std::string *code_ptr,
      ...
      if (opts.cs_gen_json_serializer) {
        if (IsUnion(field.value.type)) {
          ...
          if (IsVector(field.value.type)) {
            ...
            code += "      int _o = new " + utype_name + "[this." + camel_name +
                    ".Count];\n";
            code +=
            ...
            code += "    }\n";
            code += "    set {\n";
            code += "      this." + camel_name + " = new ArrayList<" + utype_name +
                    "Union>();\n";
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      code += "  public " + type_name + " get" + camel_name + "() { return " + field.name + "; }\n";
      code += "  public void set" + camel_name + "(" + type_name + " " + field.name + ") { this." + field.name + " = " + field.name + "; }\n";
    }
    ...
    if (parser_.root_struct_def_ == &struct_def) {
      ...
      code += "    return fbb.dataBuffer().array();\n";
      code += "  }\n";
    }
    code += "}\n\n";
  }
